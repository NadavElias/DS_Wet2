...
class RTree {
    ...
    static TNode<K, V>* arraysToTree(K* keys, V* values, int length){
        ...
        TNode<V, K>* left = arraysToTree(keys, values, middle);
        TNode<V, K>* right = arraysToTree(keys + middle + 1, values + middle + 1, length - middle - 1);
        TNode<V, K>* node = new TNode<V, K>(keys[middle], values[middle], NULL, left, right);
    ...
    static void treeToArrays(TNode<K, V>* curr, K* keys, V* values, int* index){//assumes the length of the array is correct
        ...
        (*index)++;
        treeToArrays(curr->GetRightSon(), keys, values, index);
    }
